...
def home():
    ...
    if not stored_email or stored_email == 'None':
        return redirect(f"{CROP_PROJECT_URL}/?next=chatbot")
        
    if 'session_id' not in session:
        session['session_id'] = str(uuid.uuid4())
        
    return render_template('chatbot.html', email=stored_email, chatbot_url=CHATBOT_URL, crop_project_url=CROP_PROJECT_URL, session_id=session['session_id'])

@app.route('/logout')
def logout():
    session.clear()
    return redirect(f"{CROP_PROJECT_URL}/?logout=true")

@app.route('/cron', methods=['GET'])
def cron():
    print("Cron ping received")
    return jsonify({"status": "chatbot alive ✅"})
